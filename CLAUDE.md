...
- `src/core/content.ts` — What the `content` prop takes, and the only place a caller's own *text* becomes rule text: a keyword (`empty` is `''`), a value written as CSS (a quoted string, `attr()`/`counter()`/`url()`/`var()`, or a sequence of them), or text, which is quoted and escaped. The CSS form is scanned rather than pattern-matched — one pass tracking whether it is inside a string, because `(` there is a character and outside it opens a function that has to close
