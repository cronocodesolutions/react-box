...
@cronocode/react-box is a React runtime CSS-in-JS library. The core `Box` component accepts 197 CSS props and generates CSS classes at runtime — no CSS files needed. Same prop values across components share a single CSS class.
...
- `src/core/boxStyles.ts` — All CSS property definitions (197 props). Types auto-generate from these definitions
...
- `src/core/variables.ts` — Where a value becomes a _variable_: the registry that declares `--token` in `:root` the first time something uses it (lazy, via the pending-variables queue), plus the non-colour value families (`bgImages`, `shadows`, percentages, system colours) and the `vars` grammar
...
- `src/core/filters.ts` — The nine filter functions, twice: once on `filter` and once on `backdrop-filter`. Same problem as the shadow stack — one property whose value is a _list_ — so each function sets its own custom property and every one of them writes the same composed declaration. The order is the grammar's rather than the caller's, which is what lets two elements naming the same two functions share a class, and `drop-shadow` sits last where nothing else blurs it. All eighteen layers are registered `@property … inherits: false`: `filter` inherits to nobody but a custom property does, so an unregistered `--boxBlur` would blur every descendant naming any filter of its own a second time. `dropShadow`'s scale lives in `shadows.ts` with the other three, because the `@`-colour trick is that module's
...
- **A filter's number is that function's own unit**, and there is no divider: a percentage for `brightness`, `contrast`, `grayscale`, `invert`, `saturate`, `sepia` and `backdropOpacity` (`brightness={110}` = `brightness(110%)`), degrees for `hueRotate`, px for `blur` — which also takes Tailwind's `xs`–`xxxl` scale. The nine stack into one `filter` and the nine `backdrop*` twins into one `backdrop-filter`, so `none` on any of them clears **that function only**. `dropShadow` is the ninth filter function rather than a fifth shadow layer, which is exactly the difference worth knowing: it is cast by the element's _shape_ where `shadow` draws a rectangle round its box. `maskImage` takes the `bgGradient` record and reads its alpha channel; `bgClip="text"` is inert without `color="transparent"`
- **`aspectRatio` is the one prop that takes a fraction string _and_ a division**: `'square'`/`'video'` are names for 1/1 and 16/9, `'4/3'` is written compactly and spaced out into the `4 / 3` CSS reads, and a number is a ratio as it stands (`aspectRatio={2}`). It has a `match`, so `'4:3'` emits nothing — the rule every template-type `values` follows. **`insetX`/`insetY` are `inset-inline`/`inset-block`**, the way `mx`/`my` are the margin shorthands, and every inset prop takes `'auto'` and the fractions. **An alignment can be overflow-safe**: `'safe center'` and the five others are values on all eight alignment props, and `safe center` is the alignment that cannot scroll a child out of reach. `accentColor`/`caretColor`/`colorScheme`/`fieldSizing`/`scrollbarGutter` are the native-control tier — the parts of a checkbox, a caret or a scrollbar the page does not draw. `userSelect="contain"` is deliberately **not** in the registry: Chrome computes it to `auto` (measured), and a typed value that does nothing is the one promise this library must not make
