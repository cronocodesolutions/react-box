...
export class DataGridHelper2<TRow> {
  constructor(
  ...
  private addGridGroupRow(groupColumnIndex: number, dataRows: TRow[], parentGroupValue: string = '') {
    ...
    groupEntries.forEach(([groupValue, dataRows]) => {
      ...
      const row: GridGroupRow2 = { key: rowGroupKey, isGrouped: true, count: dataRows?.length ?? 0, cells: [] };
  ...
  private addGridNormalRow(dataRow: TRow, rowIndex: number, parentGroupValue: string = '') {
    ...
    const row: GridNormalRow2 = { cells: [], key, isGrouped: false };
  ...
  private read(cols: ColumnType<TRow>[], headerRow = 0, parentKey?: Key) {
    cols.forEach((col) => {
