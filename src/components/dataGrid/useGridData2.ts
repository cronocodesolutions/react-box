...
import { GridCell2, GridDefinition, GridNormalRow2, GridRow2, Key, PinPosition, SortColumnType } from './dataGridContract';
...
export default function useGridData2<TRow>(props: Props<TRow>) {
  ...
  const helper = useMemo(
    () => new DataGridHelper2(props, leftPinnedColumns, rightPinnedColumns, columnSize, groupColumns, sortColumn, page, expandedRow),
  ...
  const resizeColumnHandler = useCallback(
    (e: React.MouseEvent, key: Key, pinned?: PinPosition) => {
      ...
      const cells = helper.dataColumns.filter((c) => leafs.includes(c.key)).map((c) => ({ key: c.key, inlineWidth: c.inlineWidth }));
  ...
  const rows = useMemo<GridRow2[]>(() => {
    ...
    const headerRow: GridNormalRow2 = {
      key: 'header',
      isGrouped: false,
      cells: [],
    };
...
export type GridData<TRow> = ReturnType<typeof useGridData2<TRow>>;
